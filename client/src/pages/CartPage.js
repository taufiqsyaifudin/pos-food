...
function CartPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (id, record) => (
        <div>
          <p>{(record.price).toLocaleString(undefined, {maximumFractionDigits:1})}</p>
        </div>
      ),
    },
    {
    ...
    <DefaultLayout>
      ...
      <div className="d-flex justify-content-end flex-column align-items-end">
        <div className="subtotal">
          <h3>
            SUB TOTAL : <b>Rp. {subTotal.toLocaleString(undefined, {maximumFractionDigits:1})}</b>
          </h3>
        </div>
      ...
      >
        ...
        <Form layout="vertical" onFinish={onFinish}>
          ...
          <div className="charge-bill-amount">
            <h5>
              SubTotal : <b>Rp. {subTotal.toLocaleString(undefined, {maximumFractionDigits:1})}</b>
            </h5>
            <h5>
              Tax : <b>Rp. {((subTotal / 100) * 10).toLocaleString(undefined, {maximumFractionDigits:1})}</b>
            </h5>
            <hr />
            <h2>
              Grand Total : <b>Rp. {(subTotal + (subTotal / 100) * 10).toLocaleString(undefined, {maximumFractionDigits:1})}</b>
            </h2>
          </div>
